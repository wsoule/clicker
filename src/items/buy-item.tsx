...
export const BuyItem : FC<BuyItemProps> = ({
  ...
  const onClick = (): void => {
    ...
    const newItem: ItemProps = {
      ...
      cost: (item.name === items.shop1.name) ? item.cost * 2 : newItemCost
    };
    ...
    setItems({
      ...
      count: {
        ...
        perClick: (item.name === items.shop1.name) ? items.count.perClick + 1 : items.count.perClick
      },
      [itemName]: newItem
    });
  };
  return (
    <>
      ...
      <Modal isOpen={!!modalMessage} onClose={onClose}>
        <ModalOverlay />
        <ModalContent>
          <ModalHeader>NEW MODAL MESSAGE</ModalHeader>
          <ModalCloseButton />
          <ModalBody>
